...
app.post('/save-word', async (req, res) => {
  ...
  try {
    ...
    if (wordExists) {
      return res.status(400).json({ message: `"${word}" already exists in the database.` });
    }
...
app.delete('/clear-words', async (req, res) => {
  ...
  try {
    ...
    await batch.commit();
    res.status(200).json({ message: 'Words cleared successfully from Firebase!' });
  } catch (error) {
...
app.get('/get-correct-words', async (req, res) => {
  ...
  try {
    const snapshot = await db.collection('users').doc(uid).collection('correctWords').get();
    const words = snapshot.docs.map((doc) => ({
      word: doc.id,
      translations: doc.data().translations,
    }));
...
app.get('/get-incorrect-words', async (req, res) => {
  ...
  try {
    const snapshot = await db.collection('users').doc(uid).collection('incorrectWords').get();
    const words = snapshot.docs.map((doc) => ({
      word: doc.id,
      translations: doc.data().translations,
    }));
...
app.patch('/promote-word', async (req, res) => {
  ...
  if (!uid || !word || !fromCollection) {
    return res.status(400).json({ message: 'Word and fromCollection are required.' });
  }
  ...
  if (!toCollection) {
    return res.status(400).json({ message: `Cannot promote from ${fromCollection}.` });
  }

  try {
    const docRef = db.collection('users').doc(uid).collection(fromCollection).doc(word);
    const doc = await docRef.get();

    if (!doc.exists) {
      return res.status(404).json({ message: `"${word}" not found in ${fromCollection}` });
    }

    const data = doc.data();

    await db.collection('users').doc(uid).collection(toCollection).doc(word).set(data);
    await docRef.delete();
...
app.get('/get-monthly-words', async (req, res) => {
  ...
  try {
    const snapshot = await db.collection('users').doc(uid).collection('expertWords').get();
    const words = snapshot.docs.map((doc) => ({
      word: doc.id,
      translations: doc.data().translations,
    }));
